...
- [ ] The gate opens with the **recommended-baseline / customize express-lane question**, the baseline path named and explained per §6.0 — never presented as the course's own configuration; the baseline path skips the per-row questions but still prints the full resolved-decision checklist.
