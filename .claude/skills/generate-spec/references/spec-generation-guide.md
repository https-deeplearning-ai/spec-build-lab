...
<!-- ALWAYS the very first section, ahead of the Decision Ledger. The pre-build gate (§6.0):
     ...
     printed and written to the build folder as `resolved-decisions.md` (a record, never an
     input). No "proceed if no response" escape (§12.10); no partial-present build (§12.11).
...
- [ ] The gate gates the build on a **printed and written resolved-decision checklist covering every row** — the fixed-name file per §6.0, cited by §7's evidence requirement — not on a single reply; answers to *some* rows do not release the build (§12.11).
