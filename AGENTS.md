...
- `/extract-build-log <run-NN>` — slices the build conversation out of Codex
...
Recording is **post-hoc**: Codex already writes every turn into
`~/.Codex/projects/<slug>/<session>.jsonl`, and `/extract-build-log` slices that
...
still preserved in Codex's transcript directory; the eval skills judge the
...
- A stateless `PreToolUse` hook (`.Codex/hooks/no-materials-during-build.sh`)
...
**Writing constraint**: any document the build agent reads (this AGENTS.md, every
...
- **The human never creates run folders. `/prepare-build` does, as its first
  ...
  4. Drop the session breadcrumb at `evals/run-NN/.session` containing the
     absolute path to the current Codex transcript JSONL. Write-once,
...
- **Version control:** per course, only `spec.md`, `materials/`, and any
  ...
  without touching `spec.md` itself; or a **generated** environment spec from
  `/generate-spec --env=<slug>`, named `spec.<slug>.md`. Everything else under
  `courses/<name>/` — `builds/`, `evals/`, scratch — is gitignored and stays
  local to the machine that ran the build. `environments/` is tracked and sits
  outside `courses/` — one profile per build target, not per course.
