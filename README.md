...
│       │   └── references/
│       │       ├── spec-generation-guide.md              # the base guide
│       │       └── spec-generation-guide.<slug>.md       # --env=<slug> overlay
...
├── environments/                      # one profile per build target, shared across courses
│   └── coding-agent-lab.md            # source doc the matching overlay was distilled from
├── templates/
...
└── courses/
    └── <course-name>/                 # one per course (created by new-course)
        ├── materials/{notebooks,transcripts}/   # course evidence ONLY
        ├── spec.md                    # the agnostic learner takeaway
        ├── spec.<slug>.md             # optional: variant (generated by --env, or hand-edited)
        ├── builds/
...
flowchart TD
    ...
    GS["<b>/generate-spec</b><br/>materials → spec.md<br/><i>--env=&lt;slug&gt; → spec.&lt;slug&gt;.md</i>"]
...
#     (or pass --env=<slug> to target a known build environment
#      -> spec.<slug>.md, leaving spec.md alone)

# 4 · start a build (call it explicitly)
/prepare-build                        # -> builds/run-01/  (the app lives here)
#     (pass the same --env=<slug> to build from that variant)
...
- Per course, only `spec.md`, `materials/`, and any `spec.<variant>.md` are
  version-controlled — a variant being either a hand-edited exploration copy of
  the canonical spec (for trying a Decision Ledger change without touching
  `spec.md`) or a generated environment spec from `/generate-spec --env=<slug>`.
  Everything else under `courses/<name>/` — `builds/` (built apps are artifacts)
  and `evals/` — is gitignored and stays local. `environments/` sits outside
  `courses/` and is tracked: one profile per build target, shared across courses.
